...
uses
  ...
  FireDAC.comp.Client, FireDAC.DApt, DateUtils;
...
begin
    try
        ...
        LANC.DT_DE := FormatDateTime('YYY-MM-DD', StartOfTheMonth(dt_filtro));
        LANC.DT_ATE := FormatDateTime('YYY-MM-DD', EndOfTheMonth(dt_filtro));
        ...
        begin
            ...
            FrmPrincipal.AddLancamento(
                      FrmLancamentos.lv_lancamento,
        ...
        lbl_saldo.Text := FormatFloat('#,##0.00', vl_rec + vl_desp);
...
procedure TFrmLancamentos.lv_lancamentoUpdateObjects(const Sender: TObject;
  const AItem: TListViewItem);
begin
    FrmPrincipal.SetupLancamento(lv_lancamento, AItem);
end;
